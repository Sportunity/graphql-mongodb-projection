...
function infoToProjection(info, context) {
  var fieldNodes = [];
  ...
  if (info.fieldNodes && info.fieldNodes.length > 0) {
    ...
    for (var i = 1; i < info.fieldNodes.length; i++) {
      for (var j = 0; j < info.fieldNodes[i].selectionSet.selections.length; j++) {
        var index = fieldNode.selectionSet.selections.findIndex(function (s) {
          return s.name.value === info.fieldNodes[i].selectionSet.selections[j].name.value;
        });
  ...
  if (!context || typeof context === 'undefined') context = (info.fieldASTs || fieldNodes)[0];
